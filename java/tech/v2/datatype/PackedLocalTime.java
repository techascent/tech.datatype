 ...
 * bytes (unsigned short): millisecond of minute
 *
...
public class PackedLocalTime {
  ...
  public static int pack(LocalTime time) {
    ...
    long millis = (long)(time.getNano() / 1_000_000.0);
    millis = (millis + (time.getSecond() * 1000));
    return create(hour, minute, (int)millis);
  }

  public static int create(int hour, int minute, int seconds, int millis) {
    return create(hour, minute, millis + (seconds * 1000));
  }

  public static int create(int hour, int minute, int millis) {
    byte m1 = (byte) (millis >> 8);
    byte m2 = (byte) millis;

    return ByteConversions.intFromBytesBE((byte)hour, (byte)minute, m1, m2);
  }
